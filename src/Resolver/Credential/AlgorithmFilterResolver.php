...
use Phayne\Saml\Credential\Criteria\AlgorithmCriteria;
use Phayne\Saml\Criteria\CriteriaSet;

/**
 * Class AlgorithmFilterResolver
 *
    ...
    public function resolve(CriteriaSet $criteriaSet, array $credentials = []): array
    {
        ...
        foreach ($criteriaSet->get(AlgorithmCriteria::class) as $criteria) {
            ...
            foreach ($credentials as $credential) {
                if (
                    (
                        null !== $credential->privateKey &&
                        $credential->privateKey->getAlgorithm() === $criteria->algorithm
                    ) || (
                        null !== $credential->publicKey &&
                        $credential->publicKey->getAlgorithm() === $criteria->algorithm
                    )
                ) {
                    $result[] = $credential;
                }

...
/**
 * Class ServiceTypeEndpointResolver
 *
...
class ServiceTypeEndpointResolver implements EndpointResolverInterface
{
    ...
    public function resolve(CriteriaSet $criteriaSet, array $candidates): array
    {
        ...
        foreach ($criteriaSet->get(ServiceTypeCriteria::class) as $criteria) {
            /** @var EndpointReference $endpointReference */
            foreach ($candidates as $endpointReference) {
                if ($endpointReference->endpoint instanceof $criteria->serviceType) {
